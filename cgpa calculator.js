...
function count(tbody){
  ...
  for(let i=0;i<tbl.rows.length;i++){
      ...
      sum2=sum2+parseFloat(tbl.rows[i].cells[1].children[0].value)
       sum1=sum1+ parseFloat(tbl.rows[i].cells[2].children[0].value)*parseFloat(tbl.rows[i].cells[1].children[0].value)
   ...
   document.getElementById("CGPAcount").innerHTML=cgpa.toFixed(2)
    ...
    let progress,color;
    color=document.getElementById("progress");
    progress=(cgpa*100)/4;
    document.getElementById("progress").style.width=progress+"%";
    let totcred=document.getElementById("totcred");
    totcred.innerHTML=sum2;
